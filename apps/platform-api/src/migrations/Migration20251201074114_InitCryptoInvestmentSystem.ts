...
export class Migration20251201074114_InitCryptoInvestmentSystem extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table "tenants" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, "name" varchar(255) not null, "slug" varchar(255) not null, "email" varchar(255) not null, "custom_url" varchar(255) null, "branding" jsonb null, "status" text check ("status" in ('active', 'suspended', 'inactive')) not null default 'active', "plan" text check ("plan" in ('trial', 'basic', 'pro', 'enterprise')) not null default 'trial', "trial_ends_at" timestamptz null, "revenue_wallets" jsonb not null, "system_fee_rate" numeric(5,2) not null default 10, "crypto_config" jsonb not null, "custom_domain" varchar(255) null);`);
